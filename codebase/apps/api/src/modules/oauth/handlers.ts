...
export async function handleSlackOAuthCallback(req: Request, res: Response): Promise<void> {
  ...
  if (oauthError || !code || !state) {
    res.redirect(integrationOAuthRedirectUrl(false, 'slack'));
    return;
  }

  const ctx = consumeIntegrationOAuthContext(res, req.headers.cookie, state);
  if (!ctx || ctx.kind !== 'chat' || ctx.provider !== 'slack') {
    res.redirect(integrationOAuthRedirectUrl(false, 'slack'));
    return;
  }
  ...
  try {
    ...
    await IntegrationConnection.findOneAndUpdate(
      { workspaceId: ctx.workspaceId, providerKey: 'slack' },
      {
        status: 'connected',
        externalAccountId: tokens.externalAccountId ?? 'slack-workspace',
        settings: { mode: 'live', provider: 'slack' },
      },
      { upsert: true },
    );
    await saveTokens(ctx.workspaceId, 'slack', {
      accessToken: tokens.accessToken,
      refreshToken: tokens.refreshToken,
      expiresAt: tokens.expiresAt,
    });
    res.redirect(integrationOAuthRedirectUrl(true, 'slack'));
  } catch (err) {
    log('oauth', 'slack callback failed', {
      error: err instanceof Error ? err.message : String(err),
    });
    res.redirect(integrationOAuthRedirectUrl(false, 'slack'));
  }
